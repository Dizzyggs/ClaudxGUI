...
const PermissionAlert: React.FC<PermissionAlertProps> = ({ 
  ...
  const getIcon = () => {
    if (missingPermissions.includes('Write Files')) {
      return (
    ...
    if (missingPermissions.includes('Edit Files')) {
      return (
        <svg width="20" height="20" viewBox="0 0 24 24" fill="none">
    ...
    if (missingPermissions.includes('Delete Files')) {
      return (
        <svg width="20" height="20" viewBox="0 0 24 24" fill="none">
    ...
    if (missingPermissions.includes('Run Commands')) {
      return (
        <svg width="20" height="20" viewBox="0 0 24 24" fill="none">
  ...
  const getPermissionText = () => {
    if (missingPermissions.length === 1) {
      return `Enable ${missingPermissions[0]}`;
    } else if (missingPermissions.length === 2) {
      return `Enable ${missingPermissions.join(' & ')}`;
    } else {
      return `Enable ${missingPermissions.length} Permissions`;
    }
  };
  ...
  return (
    <div className="permission-alert">
      <div className="permission-alert-content">
        <div className="permission-alert-icon">
        ...
        <div className="permission-alert-text">
          ...
          <p>This action requires <strong>{missingPermissions.join(', ')}</strong> permission{missingPermissions.length > 1 ? 's' : ''}.</p>
        </div>
      </div>
      
      <button 
        className="permission-alert-button"
        onClick={onOpenSettings}
        aria-label={getPermissionText()}
      >
